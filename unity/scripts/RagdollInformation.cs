...
public class RagdollInformation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            string header = "Time, Body Part Collided with: , Impact Force (N), Relative Velocity (m/s), Distance from Joint (m)\n";
    ...
    {   
        ...
        {
            // Anchor position in local space
            Vector3 localAnchor = joint.anchor;
            // To get the world position of the anchor, transform it from local to world space:
            ...
            distanceToImpactCoords = worldAnchor - worldPos;
            distanceToImpact = Mathf.Sqrt(Mathf.Pow(distanceToImpactCoords[0],2) + Mathf.Pow(distanceToImpactCoords[1],2) + Mathf.Pow(distanceToImpactCoords[2],2));
            string data = $"{timeStamp}, {gameObject.name}, {collisionForce}, {collisionVelocity}, {distanceToImpact}\n";
